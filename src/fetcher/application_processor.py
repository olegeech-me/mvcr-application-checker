...
class ApplicationProcessor:
    ...
    async def start_processing(self, app_number):
        ...
        async with self.lock:
            logger.info(f"Locking {app_number} for processing")
            self.processing_apps.add(app_number)

    async def end_processing(self, app_number):
        """Mark an application number as done processing"""
        async with self.lock:
            if app_number in self.processing_apps:
                logger.info(f"Unlocking {app_number}, processing finished")
    ...
    async def _process_request(self, message, request_type):
        ...
        if await self.is_processing(app_details["number"]) and not retry_count:
            logger.info(
